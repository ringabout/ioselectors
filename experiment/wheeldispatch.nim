...
proc `len`*(t: Timer): int =
  t.wheel.taskCounter

proc clear*(t: var Timer) =
  discard

proc futurecb(args: pointer = nil) =
  if args != nil:
    let fut = cast[Future[void]](args)
    fut.complete()

proc push*(t: var Timer, item: (int, Future[void])) =
  let (timeout, fut) = item
  var event = initTimerEvent(futurecb, cast[pointer](fut))
  discard t.add(event, timeout)
...
proc processTimers*(
  ...
  var count = p.timers.queue.len
  let now = getMonoTime()

  while count > 0 and p.timers.queue[0].finishAt <= now:
    let 
      item = p.timers.queue.pop
      distance = item.finishTick - p.timers.wheel.currentTime
    ...
    p.timers.update(timeout)
    dec count
    didSomeWork = true

  if p.timers.queue.len == 0:
    result = none(int)
  else:
    let millisecs = (p.timers.queue[0].finishAt - getMonoTime()).inMilliseconds
...
when defined(windows) or defined(nimdoc):
  ...
  proc addRead*(fd: AsyncFD, cb: Callback) =
    ...
    ##
    ## If you use this function, you don't need to use ../../wheeldispatch.recv()
    ## or ../../wheeldispatch.accept(), because they are using IOCP, please use
  ...
  proc addWrite*(fd: AsyncFD, cb: Callback) =
    ...
    ## asynchronous on Windows).
    ##
    ## If you use this function, you don't need to use ../../wheeldispatch.send()
    ## or ../../wheeldispatch.connect(), because they are using IOCP, please use
...
proc sleepAsync*(ms: int | float): owned(Future[void]) =
  ...
  let p = getGlobalDispatcher()
  p.timers.push((ms.int, retFuture))
  return retFuture
...
proc withTimeout*[T](fut: Future[T], timeout: int): owned(Future[bool]) =
  ...
  var retFuture = newFuture[bool]("../../wheeldispatch.`withTimeout`")
