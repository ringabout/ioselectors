...
Async traceback:
  tasync_traceback\.nim\(\d+?\)\s+?tasync_traceback
  ../../wheeldispatch\.nim\(\d+?\)\s+?waitFor
  ../../wheeldispatch\.nim\(\d+?\)\s+?poll
    ## Processes asynchronous completion events
  ../../wheeldispatch\.nim\(\d+?\)\s+?runOnce
  ../../wheeldispatch\.nim\(\d+?\)\s+?processPendingCallbacks
